...
//String reader[];
//String text[] ; 
String[]  reader = { "0", "0", "0", "0", "0" };
String[]  text = { "0", "0", "0", "0", "0" };


int[] scores= { 0,0,0,0,0 }; //array da pontuação em valores inteiros
int[] leaderboard= { 0,0,0,0,0 }; //5 melhores pontuações
...
text = new String[5];
  ...
  //Tipo de letra
  crackmanFont=createFont("COPRGTL.TTF", 18);
  lotFont=createFont("COPRGTL.TTF", 18);
...
void draw() {
  ...
  msgClient=c.readString();
   if( (msgClient.indexOf ("commande") != -1) && (msgClient.length()>=msgClient.indexOf ("servo")+10) ) {
    ...
    ////////////////////////////////////////
      if (( Value.charAt(0) == 'm' )&&((stage=="game"||stage=="pause"))) {soundToggle();Value = "--";}
      if (( Value.charAt(0) == 'p' )&&((stage=="game"||stage=="pause"))) {println("toto");pauseToggle();Value = "--";}
    ...
    //Leaderboard
    if (Value.charAt(0)=='w') stage="leaderboard";

    //SORTIR DU JEU
    if (Value.charAt(0)=='q') exit();
  }
  if (stage=="leaderboard") {
    if (Value.charAt(0)=='r') stage="menu";
  }
  if (stage=="death") {
    if (Value.charAt(0)=='s') {
      direction="right";
  ...
  if (stage=="leaderboard") {
    ...
    fill(255, 255, 0);
    textSize(40);
    text("Best score : "+bestscore+" points", 100, 200);
    //2 LUGAR
    fill(255, 255, 0);
    textSize(40);
    text("Last Score : "+score+" points", 100, 240);
    //3 LUGAR
    //fill(255, 255, 0);
    //textSize(40);
    //text("3ème Place : "+leaderboard[2]+" points", 100, 280);
    ////4 LUGAR
    //fill(255, 255, 0);
    //textSize(40);
    //text("4ème Place : "+leaderboard[3]+" points", 100, 320);
    ////5 LUGAR
    //fill(255, 255, 0);
    //textSize(40);
    //text("5ème Place : "+leaderboard[4]+" points", 100, 360);
    buttonHighlight();
  }
  if (stage=="ready") {
    background(0);
    fill(255, 255, 0);
    textSize(60);
    text("PRÊT !", (width/2)-130, height/2);
    textSize(15);
    text("Projet requête HTTP PACMAN : ", 80, 480);
    text("L.Chastain : Ac.LIMOGES / K.THOMAS : Ac.CRETEIL", 80, 500);
  ...
  if (stage=="death") {
    ...
    fill(255, 0, 0);
    textSize(80);
    text("PERDU", (width/2)-260, height/2);
    textFont(crackmanFont);
    fill(255, 255, 0);
    textSize(40);
    ...
    if (isHighscore()) {
      fill(0, 255, 0);
      textSize(30);
      text("Vous avez atteint le classement!", (width/2)-330, (height/2)+50);
    }
    buttonHighlight();
   // writeToFile();
  }
  if (stage=="victory") {
    ...
    fill(0, 255, 0);
    textSize(60);
    text("VICTOIRE !", (width/2)-170, height/2);
    textFont(crackmanFont);
    fill(255, 255, 0);
    textSize(15);
    text("Projet requête HTTP PACMAN : ", 80, 480);
  ...
  if (stage=="pause") {
    ...
    fill(255, 255, 0);
    textSize(40);
    text("Pause", centroX(6)-8, centroY(4)+14);
  }
}
...
public void buttonHighlight() {
  if (stage=="menu") {
    ...
    //Botão PLAY
      fill(255, 255, 0);
      textSize(20);
      text("s : Démarrer", 80, 320);
    //Botão LEADERBOARD
      text("w : High Score", 80, 350);
    //Botão EXIT
      text("q : Sortie", 80, 380);
      text("p : Pause", 80, 410);
      text("m : Activer / désactiver le son", 80, 440);
      
      text("Envoyez une requête http pour commencer", 80, 200); 
      text("requête http : " + Value, 80, 230) ;
      text("mon IP : " + s.ip(), 80, 260) ;
      textSize(15);
      text("Projet requête HTTP PACMAN : ", 80, 480);
      text("L.Chastain : Ac.LIMOGES / K.THOMAS : Ac.CRETEIL", 80, 500);
  }
  ...
  if (stage=="death") {
      ...
      fill(255, 255, 0);
      textSize(40);
      text("w : High Score", 300, 370);
      text("s : Rejouer", 300, 410);
      text("q : Sortie", 300, 450);
  }
  if (stage=="victory") {
      fill(255, 255, 0);
      textSize(40);
