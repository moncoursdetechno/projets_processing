...
{int rows = 5;int cols = 10;
 ...
 int speedUpdate = 40;
 float velocityX = 8; int velocityY = 8;
 int deltaX; int deltaY;
 ArrayList<SpaceInvader> invaders;
 ArrayList<Laser> lasers;

 Fleet() 
 {deltaX = width / 10 + 35;deltaY = height / 10 + 70;
 ...
 void update() 
 {updateLasers();
  if (frameCount%speedUpdate != 0) return;
  shotLaser();
  positionFlag = !positionFlag;
   if (boundaryContact) 
   {boundaryContact = false;
    for (SpaceInvader spaceInvader : invaders) 
    {spaceInvader.moveY(velocityY);} return;}
    for (SpaceInvader spaceInvader : invaders)
 ...
 int randomInvaderIndex = (int) random(invaders.size()) - 1;
    randomInvaderIndex = constrain(randomInvaderIndex, 0,invaders.size() - 1);
    SpaceInvader shootingInvader = invaders.get(randomInvaderIndex);
    lasers.add(new Laser(shootingInvader.location.x,shootingInvader.location.y));}
