...
{PVector location;
 PVector hitboxNE;PVector hitboxSW;
 boolean aLive;boolean debugE;
 ...
 {this.hitboxNE = hitboxNE;this.hitboxSW = hitboxSW;}
 ...
 {PVector hitboxcopNE = gethitboxNE();
  ...
  if (hitboxcopNE.y > otherHitboxcopNE.y + other.hitboxSW.y) return false;return true;}
  
 boolean contact(PVector loc, int size)
 {PVector hitboxcopNE = gethitboxNE();
  if (hitboxcopNE.x+hitboxSW.x < loc.x) return false;
  if (hitboxcopNE.x > loc.x + size) return false;
  if (hitboxcopNE.y+hitboxSW.y < loc.y) return false;
  if (hitboxcopNE.y > loc.y + size) return false;return true;}
